...
namespace Assets.Scripts.Game
{
    public class VirusController : MonoBehaviour, IMonoBehaviour
    {
        ...
        private void AddNewVirus(BlobDto virus, int worldSize)
        {
            ...
            virusObject.UpdateVirus(virus, worldSize);
            SaveVirus(virusObject, virus);
        }

        private void UpdateVirus(BlobDto virusDto, int worldSize)
        {
            if (_virusObjects.ContainsKey(virusDto.Id))
            {
                _virusObjects[virusDto.Id].UpdateVirus(virusDto, worldSize);
            }
        }
